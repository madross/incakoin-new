    ...
    model(0)
{
	...
	// Coin Control
     ui->lineEditCoinControlChange->setFont(GUIUtil::IncaKoinAddressFont());
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        formatted.append(tr("<b>%1</b> to %2 (%3)").arg(IncaKoinUnits::formatWithUnit(IncaKoinUnits::BTC, rcp.amount), Qt::escape(rcp.label), rcp.address));
    #else
		formatted.append(tr("<b>%1</b> to %2 (%3)").arg(IncaKoinUnits::formatWithUnit(IncaKoinUnits::BTC, rcp.amount), rcp.label.toHtmlEscaped(), rcp.address));
	#endif
	}
    ...
    case WalletModel::AmountWithFeeExceedsBalance:
        QMessageBox::warning(this, tr("Send Coins"),
            ...
            arg(IncaKoinUnits::formatWithUnit(IncaKoinUnits::BTC, sendstatus.fee)),
            QMessageBox::Ok, QMessageBox::Ok);
        break;
...
bool SendCoinsDialog::handleURI(const QString &uri)
{
    ...
    // URI has to be valid
    if (GUIUtil::parseIncaKoinURI(uri, &rv))
    {
        CIncaKoinAddress address(rv.address.toStdString());
        if (!address.IsValid())
            return false;
        pasteEntry(rv);
        return true;
...
void SendCoinsDialog::updateDisplayUnit()
{
    if(model && model->getOptionsModel())
    {
        ...
        ui->labelBalance->setText(IncaKoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), model->getBalance()));
    }
}
...
void SendCoinsDialog::coinControlChangeChecked(int state)
{
    ...
    {
        if (state == Qt::Checked)
            CoinControlDialog::coinControl->destChange = CIncaKoinAddress(ui->lineEditCoinControlChange->text().toStdString()).Get();
        else
...
void SendCoinsDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        CoinControlDialog::coinControl->destChange = CIncaKoinAddress(text.toStdString()).Get();
        ...
        else
        {
            ...
            else
            {
                CPubKey pubkey;
                CKeyID keyid;
                CIncaKoinAddress(text.toStdString()).GetKeyID(keyid);   
