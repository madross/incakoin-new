...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    GenerateIncaKoins(fGenerate, pwalletMain);
    return Value::null;
}
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "IncaKoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "IncaKoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "IncaKoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "IncaKoin is downloading blocks...");
