...
const string strMessageMagic = "IncaKoin Signed Message:\n";
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime)
{
    ...
    // IncaKoin: reward for coin-year is cut in half every 64x multiply of PoS difficulty
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal IncaKoin miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in IncaKoin
    bool fStrictPayToScriptHash = true; // Always active in IncaKoin
    ...
    // ppcoin: fees are not collected by miners as in IncaKoin
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (pblock->IsProofOfStake())
    {
        ...
        if (!CheckProofOfStake(pblock->vtx[1], pblock->nBits, hashProofOfStake, pblock->nTime))
        {
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "IncaKoin", CClientUIInterface::MSG_WARNING);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        nStakeMinAgeOld = 2 * 60 * 60; // test net min age is 2 hours
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Por unos caminos hechos en lineas nacio el oro de Inca.";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:IncaKoin genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("0x9661a7148d2947b1be2a1644fc97ee79def9f995867c993ee7061b610cb66525"));
        block.print();
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    if (fProofOfStake)  // attempt to find a coinstake
    {
        ...
        {
            if (pwallet->CreateCoinStake(*pwallet, pblock->nBits, nSearchTime-nLastCoinStakeSearchTime, txCoinStake, pblock->nTime))
            {
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("IncaKoinMiner : proof-of-work not meeting target");

    //// debug print
    printf("IncaKoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("IncaKoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("IncaKoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadIncaKoinMiner(void* parg);

static bool fGenerateIncaKoins = false;
...
void IncaKoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateIncaKoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateIncaKoins) && !fProofOfStake)
                return;
        }
        ...
        printf("Running IncaKoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadIncaKoinMiner(void* parg)
{
    ...
    try
    {
        ...
        IncaKoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadIncaKoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadIncaKoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadIncaKoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateIncaKoins(bool fGenerate, CWallet* pwallet)
{
    fGenerateIncaKoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateIncaKoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d IncaKoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadIncaKoinMiner, pwallet))
                printf("Error: NewThread(ThreadIncaKoinMiner) failed\n");
            Sleep(10);
        }
    }
}
